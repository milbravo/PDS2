...
bool Locadora::validarCPF(const std::string& cpf) {
...
void Locadora::cadastrarFilmeFita(int quantidade, int codigo, const std::string &titulo) {
    estoqueFilmes.push_back(new Fita(codigo, titulo, quantidade, true));
    std::cout << "Filme " << codigo << " cadastrado com sucesso.\n";
}

void Locadora::cadastrarFilmeDVD(int quantidade, int codigo, const std::string &titulo, const std::string &categoria) {
...
void Locadora::removerFilme(int codigo) {
    auto it = std::remove_if(estoqueFilmes.begin(), estoqueFilmes.end(),
                             [codigo](const Filme *filme) { return filme->getCodigo() == codigo; });

    if (it != estoqueFilmes.end()) {
        delete *it;
        estoqueFilmes.erase(it, estoqueFilmes.end());
...
void Locadora::listarFilmes(char ordenacao) const {
    std::vector<Filme *> filmesOrdenados = estoqueFilmes;

    if (ordenacao == 'C') {
        std::sort(filmesOrdenados.begin(), filmesOrdenados.end(),
                  [](const Filme *a, const Filme *b) { return a->getCodigo() < b->getCodigo(); });
    } else if (ordenacao == 'T') {
        std::sort(filmesOrdenados.begin(), filmesOrdenados.end(),
                  [](const Filme *a, const Filme *b) { return a->getTitulo() < b->getTitulo(); });
    }
...
void Locadora::removerCliente(const std::string &cpf) {
    auto it = std::remove_if(clientes.begin(), clientes.end(),
                             [cpf](const Cliente &cliente) { return cliente.getCPF() == cpf; });
...
void Locadora::listarClientes(char ordenacao) const {
    std::vector<Cliente> clientesOrdenados = clientes;

    if (ordenacao == 'C') {
        std::sort(clientesOrdenados.begin(), clientesOrdenados.end(),
                  [](const Cliente &a, const Cliente &b) { return a.getCPF() < b.getCPF(); });
    } else if (ordenacao == 'N') {
        std::sort(clientesOrdenados.begin(), clientesOrdenados.end(),
                  [](const Cliente &a, const Cliente &b) { return a.getNome() < b.getNome(); });
    }
...
void Locadora::alugarFilme(const std::string &cpf) {
    ...
    if (cliente != nullptr) {
        if (!clientePossuiLocacao(cpf)) {
            ...
            while (std::cin >> codigo) {
                ...
                if (filme != nullptr) {
                    if (filme->getQuantidade() > 0) { // Verifica se há filmes disponíveis
...
void Locadora::devolverFilme(const std::string &cpf, int dias) {
    Cliente *cliente = buscarCliente(cpf);
    if (cliente != nullptr) {
        if (clientePossuiLocacao(cpf)) {
...
double Locadora::calcularTotalPagar(const std::vector<int> &locacoes, int dias) const {
...
Filme* Locadora::buscarFilme(int codigo) const {
...
Cliente* Locadora::buscarCliente(const std::string &cpf) const {
...
bool Locadora::clientePossuiLocacao(const std::string &cpf) const {
